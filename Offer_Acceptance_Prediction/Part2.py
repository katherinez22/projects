...
def profit(prob1, amount, rate, term, cost_rate):
    """
    Compute the expected profit from each customer.
    """
    profit = np.array(prob1) * np.array(amount) * (np.array(rate)/100-cost_rate) * np.array(term) / 12
    return np.mean(profit)


def rateFinder(x_opt, clf, start_point, end_point, cost_rate):
    """
    Find an optimal rate that maximizes the expected profit.
    """
    ...
    rateList = np.linspace(start_point, end_point, 1100, endpoint=False)  # step-size: 0.01
    for rate in rateList:
        ...
        amount = x_opt[:, 2]
        term = x_opt[:, 4]
        profitList.append(profit(prob1, amount, rate, term, cost_rate))
...
if __name__ == "__main__":
    ...
    rate, value, prob, rateList, profitList = rateFinder(x_opt, clf, 1, 12, 0.0003)
    print "The single rate that maximizes the expected profit is: %f" % rate
    ...
    rate_s, value_s, prob_s, rateList_s, profitList_s = rateFinder(x_opt_s, clf, 1, 12, 0.0003)
    rate_n, value_n, prob_n, rateList_n, profitList_n = rateFinder(x_opt_n, clf, 1, 12, 0.0003)
    ...
    ############# Data Visualization
    # Visualize Expected Profit as a Function of the Price
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(rateList, profitList)
    plt.xlabel("Price")
    plt.ylabel("Expected Profit")
    plt.title("Expected Profit as a Function of the Price")
    plt.savefig('expected_profit.png')

    # Visualize Expected Profit as a Function of the Price in Price Sensitive Group
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ...
    # Visualize Expected Profit as a Function of the Price in Price Non-Sensitive Group
    fig = plt.figure()
    ax = fig.add_subplot(111)
